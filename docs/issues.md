...
| Games run poorly with many slowdowns, but I recall them working fine some time ago. | A) You enabled input lag reduction option or B) you are using a very weak power adapter. Please use an official power supply or a compatible one with a voltage of 5.1V to 5.25V. |
